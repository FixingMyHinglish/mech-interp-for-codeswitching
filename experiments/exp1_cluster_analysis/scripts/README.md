# COMP0087 Mech-Interp Pipeline
This repository contains a lightweight neuron-analysis pipeline for multilingual activation exports under `mech_interp/models/`.
The main clustering entry point is:
- `mech_interp/models/pipeline/cluster_analysis.py`
It clusters one layer of neuron summaries and writes both a CSV summary and a Markdown README into `04_analysis/`.
## Directory layout
Each model/language run is expected to live under a work directory like:
```text
mech_interp/models/<model>/<language>/
```
Typical files and folders:
- `01_raw/full_neuron_activations.jsonl.gz`
- `02_bundle/`
- `03_cleaned/`
- `04_analysis/`
